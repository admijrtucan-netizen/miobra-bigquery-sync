...
export function toWorkforceRow(estimation: MiobraWorkforceEstimation): WorkforceRow {
  return {
    ...
    last_audit_dt_tm: (estimation as any).last_audit_dt_tm ?? null,
    total_estimated_amount: (estimation as any).total_estimated_amount ? parseFloat((estimation as any).total_estimated_amount) : null,
    total_amount: (estimation as any).total_amount ? parseFloat((estimation as any).total_amount) : null,
    ...
    real_lending_amount: (estimation as any).real_lending_amount ? parseFloat((estimation as any).real_lending_amount) : null,
    real_lending_refund_amount: (estimation as any).real_lending_refund_amount ? parseFloat((estimation as any).real_lending_refund_amount) : null,
    real_retention_amount: (estimation as any).real_retention_amount ? parseFloat((estimation as any).real_retention_amount) : null,
    ...
    is_resendable: (estimation as any).is_resendable ?? null,
    has_linked_collection_estimates: (estimation as any).has_linked_collection_estimates ?? null,
    requires_approval_confirmation: (estimation as any).requires_approval_confirmation ?? null,
